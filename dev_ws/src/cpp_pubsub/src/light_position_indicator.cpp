...
public:
  MinimalSubscriber()
  : Node("light_indicator")
  {    
    parse_parameters();
    this->declare_parameter<int>("threshold", 230);
...
private:
  ...
  void topic_callback(const sensor_msgs::msg::Image::ConstSharedPtr msg) const
  {
    ...
    cv::Mat cvImgThresh;
    cv::threshold( greyMat, cvImgThresh , darknessThreshold_, 255, 0 );     // 255 is max value, 0 indicates binary thresholding
    
    cv::Moments mom = cv::moments(cvImgThresh, true);
    RCLCPP_INFO(this->get_logger(), "Moments: = [%f, %f, %f]", mom.m10, mom.m00, mom.m01);

    if (mom.m00 > 0.1) {
        cv::Point p(mom.m10/mom.m00, mom.m01/mom.m00);     
        cv::circle(cvImgThresh, p, 5, cv::Scalar(128,0,0), -1);

        message.data = "Center of Gravity coordinates: = [" + std::to_string(p.x) + ", " + std::to_string(p.y) + "]";
        // RCLCPP_INFO(this->get_logger(), "Center of Gravity coordinates: = [%d, %d]", p.x, p.y);
    } else {
        message.data = "No bright light detected.";
        // RCLCPP_INFO(this->get_logger(), "No bright light detected.");
    }

    // TODO: Probably better to send a 2-integer array with the coordinates, and only publish when a light is detected.
    // This is easier to parse for another node
    ...
    img_bridge = cv_bridge::CvImage(header, sensor_msgs::image_encodings::MONO8, cvImgThresh);   // output GRAYSCALE (mono8) image; input is BGR8
